...
class FedAvgHFLASH(Strategy):
    ...
    def configure_fit(
        ...
        client_check = True  # to be added to the fit config !?
        while client_check:
            clients = client_manager.sample(
                num_clients=sample_size, min_num_clients=min_num_clients
            )
            for client in clients:
                print(client.cid)
                if int(client.cid) % 5 == 0:
                    client_check = False
                    log(logging.INFO, f"Client selected correctly")
                    break
            if client_check:
                log(logging.INFO, f"Client not selected correctly, retrying")
    ...
    def aggregate_fit(
        ...
        if server_round == 1:
            ...
            layer_sensitivity = np.amin(layer_sparsity, axis=1)
