...
from project.utils.utils import get_clients_window_range
...
class CidAboveNCriterion:
    ...
    def __init__(self, cid_threshold: int = 100):
    ...
    def select(self, client: ClientProxy, num_client: int = 100) -> bool:
        ...
        return int(client.cid) < self.cid_threshold
...
class FedAvgCFLASH(Strategy):
    ...
    def __init__(
        ...
        *,
        ...
        self.window_training_rounds = 200
    ...
    def configure_fit(
        ...
        subset_training_rounds = 25
        num_subsets = 4
        num_clients = 100

        # lower_bound = ((server_round // subset_training_rounds) * subset_training_rounds)% num_clinets
        # upper_bound = lower_bound + subset_training_rounds

        lower_bound, upper_bound = get_clients_window_range(
            current_round=server_round, num_clients=num_clients
        )

        clients = client_manager.sample(
            num_clients=sample_size,
            min_num_clients=min_num_clients,
            # criterion=CidWindowCriterion(
            #     upper_bound=upper_bound, lower_bound=lower_bound
            # ),
        )

        print(
            f"Round {server_round}: Selected clients with cid in [{lower_bound},"
            f" {upper_bound})"
        )
    ...
    def configure_evaluate(
        ...
        if server_round > self.window_training_rounds:
            # sample the clients from the window corrisponding to the previous window
            lower_bound, upper_bound = get_clients_window_range(
                current_round=server_round - self.window_training_rounds
            )
            clients += client_manager.sample(
                num_clients=sample_size,
                min_num_clients=min_num_clients,
    ...
    def aggregate_evaluate(
        ...
        if self.evaluate_metrics_aggregation_fn:
            ...
            # Extract window_test_acc and prev_window_test_acc
            window_test_acc = []
            prev_window_test_acc = []
            for _, metrics in eval_metrics:
                if "window_test_acc" in metrics:
                    window_test_acc.append(metrics["window_test_acc"])
                if "prev_window_test_acc" in metrics:
                    prev_window_test_acc.append(metrics["prev_window_test_acc"])
